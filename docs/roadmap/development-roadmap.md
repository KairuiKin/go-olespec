...
- Deliverables:
  ...
  - Corpus replay tool under `cmd/` for batch validation. (initial CLI + baseline diff/gate + file-set/partial gates + error-code regression gates + trend summary + trend gates + auto trend save + auto trend prune 已落地：`cmd/corpus-replay`)
