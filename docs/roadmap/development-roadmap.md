...
- Deliverables:
  ...
  - Corpus replay tool under `cmd/` for batch validation. (initial CLI + baseline/latest diff/gate + file-set/partial/warning gates + error-code regression gates + trend summary + trend gates + auto trend save + auto trend prune + report-files policy + partial/warning trend gates 已落地：`cmd/corpus-replay`)
