...
func parseReportFilesPolicy(v string) (string, error) {
	...
	case "none":
		return "none", nil
	default:
		return "", fmt.Errorf("invalid report-files %q, expected all|failed|success|partial|issues|warnings|clean|none", v)
	}
}
...
func parseReportSortPolicy(v string) (string, error) {
	...
	case "artifacts-failed-desc":
		return "artifacts-failed-desc", nil
	case "warnings-desc":
	...
	default:
		return "", fmt.Errorf("invalid report-sort %q, expected path|duration-desc|size-desc|artifacts-desc|artifacts-failed-desc|warnings-desc|error-code|failed-first", v)
	}
}
...
func applyReportFilePolicy(report *replayReport, policy, sortBy string, offset, limit int) {
	...
	case "size-desc":
		sort.SliceStable(report.Files, func(i, j int) bool {
			if report.Files[i].Size == report.Files[j].Size {
				return report.Files[i].Path < report.Files[j].Path
			}
	...
	case "artifacts-failed-desc":
		sort.SliceStable(report.Files, func(i, j int) bool {
			if report.Files[i].ArtifactsFail == report.Files[j].ArtifactsFail {
				return report.Files[i].Path < report.Files[j].Path
			}
			return report.Files[i].ArtifactsFail > report.Files[j].ArtifactsFail
		})
	case "warnings-desc":
		sort.SliceStable(report.Files, func(i, j int) bool {
			if report.Files[i].Warnings == report.Files[j].Warnings {
				return report.Files[i].Path < report.Files[j].Path
			}
	...
	if offset > 0 {
		if offset >= len(report.Files) {
			report.Files = nil
		} else {
			report.Files = report.Files[offset:]
		}
	}
